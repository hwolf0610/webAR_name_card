...
todoRoutes.route('/applogin').post(function (req, res) {
...
todoRoutes.route('/scoresGlobalshow').post(function (req, res) {
    ...
    // Scores.find(
        ...
        if (err) {
            ...
            // var swap = {}, cart = []
            // swap["_id"] = response[0]._id
            // swap["email"] = response[0].email
            // swap["cretedates"] = response[0].cretedates
            // swap["category"] = response[0].category
            // swap["scores"] = response[0].scores
            // swap["name"] = response[0].name
            // cart.push(swap);            
            // for (var i = 1; i < response.length; i++) {
            //     for (var j = 0; j < i; j++) {
            //         if (response[i].category = response[j].category && response[i].scores > response[j].scores) {
            //             swap._id = response[i]._id
            //             swap.email = response[i].email
            //             swap.cretedates = response[i].cretedates
            //             swap.category= response[i].category
            //             swap.scores = response[i].scores
            //             swap.name = response[i].name
            //             cart.push(swap)
            //         }
            //     }
            // }           
            ...
            response.map((element, index) => {
                Scores.find({"category" : element._id, "scores" : element.max_data}, function(err, response1) {
                    ret_data.push(response1[0]);   
